-- 1. create restaurant table under stage location, with all text value + audit column for copy command
-- 2. Stream object for stage table to capture the changes. 
-- 3. run copy command to load the data into stage-restaurant table.
-- 4. the restaurant table where data types are defined (clean schema)
-- 5. stream object for the clean schema restaurant table 
-- 6. merge data from the stage stream to clean schema table.
-- 7. now define dim table for restaurant.
-- 8. merge data into the consumption schema table from the clean schema's stream.
...
from (
    select 
     ...
     from @stage_sch.csv_stg/initial/restuarant/restaurant-delhi+NCR.csv t
)
