-- ENTITY : LOCATION
-- 1. create a table in the stage schema for the entity
-- 2. create a stream for that stage_sch table (append only)
-- 3. copy data into the stage_sch table from the internal stage using copy into
-- 4. create a table in the clean schema for the entity
-- 5. create a stream for that table also
-- 6. copy that data from the stage_sch table's stream to the clean_sch table using MERGE INTO
-- 7. create a comsumption schema table for the entity.
-- 8. copy the data from the clean_sch table's stream to the consumption_sch using the merge.

use role sysadmin;
use schema swiggy.stage_sch;

--drop table stage_sch.location ;
--drop stream stage_sch.location_stm;
...
)comment = 'this is a table in a stage schema for location entity; data copied from the internal stage' ;
...
file_format = (format_name = 'stage_sch.csv_format')
...
comment = 'Location table at the clean schema level; data to this table is merge from the stream of the stage level table';
...
WHEN matched AND (
    ...
    target.state_code != source.statecode OR
    ...
    UPDATE SET
        ...
        target.state_code = source.statecode,
